...
if SAVE_OUTPUT_VIDEO:
    ...
    if not writer.isOpened():
        cap.release()
        raise RuntimeError(f"処理結果動画の保存先を開けません: {output_video_path}")
...
print(f"Webカメラ準備完了（入力・保存）: {width}x{height}, FPS={fps:.1f}")
print(f"画像処理解像度: {PROCESS_WIDTH}x{PROCESS_HEIGHT}")
...
while True:
    ...
    # カメラから入力された元解像度のフレームをそのまま保存する
    if raw_writer is not None:
        raw_writer.write(frame)

    # 画像処理用にだけ640×480へ縮小する
