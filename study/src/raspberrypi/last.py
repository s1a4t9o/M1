...
# 復帰後、最も移動しやすい加圧セルを探索するときの時間
RECOVERY_SEARCH_PRESSURIZE_SEC = 3.0
RECOVERY_SEARCH_IDLE_SEC = 3.0
RECOVERY_CONTROL_CHANGE_STABLE_FRAMES = 5
...
# 各ユニットごとの減圧閾値[kPa]
# 各ユニット内の3セルは、そのユニットに設定された閾値を共通で使用する。
...
def calculate_marker_centroid(markers):
    """IDが付いた全マーカーの重心座標(x, y)を返す。"""
    valid_markers = [
        m for m in markers
        if m["assigned_id"] != -1
    ]
    ...
    centroid_x = sum(m["center"][0] for m in valid_markers) / len(valid_markers)
    centroid_y = sum(m["center"][1] for m in valid_markers) / len(valid_markers)
...
def start_stall_recovery(sequence_state):
    """
    加圧・減圧セルの組み合わせが10サイクル変わらなかった場合の
    復帰動作を開始する。
    ...
    print(
        f"加圧・減圧セルが{MAX_STALLED_CYCLES}サイクル変わらなかったため、"
        f"復帰動作を開始します"
    )
    print(
...
def start_recovery_search(start_id, markers):
    """
    復帰加圧後の探索制御を開始する。

    現在の開始IDから加圧7セル・減圧9セルを決め、まず減圧指示を送る。
    その後、加圧7セルを1つずつ試して重心の左方向移動量を比較する。
    """
    pressurize_cells, depressurize_cells = make_cells_from_start_id(start_id)
    start_centroid = calculate_marker_centroid(markers)

    print("復帰後の加圧セル探索を開始します")
    print(f"探索対象の加圧セル: {pressurize_cells}")
    print(f"減圧セル: {depressurize_cells}")
    send_cell_command(depressurize_cells=depressurize_cells)

    return {
        "reference_start_id": start_id,
        "pressurize_cells": pressurize_cells,
        "depressurize_cells": depressurize_cells,
        "phase": "wait_after_depressurize",
        "phase_start_time": time.time(),
        "test_index": 0,
        "test_start_centroid_x": (
            start_centroid[0]
        ...
        "scores": {},
        "best_cell": None,
        "change_candidate_start_id": None,
        "change_candidate_count": 0,
    }


def run_recovery_search(search_state, markers):
    """
    復帰後の探索制御を進める。

    各候補セルを「3秒加圧→3秒Idle」で試す。加圧前後のマーカー重心X座標
    を比較し、重心を画像左方向へ最も動かした1セルを選ぶ。そのセルを
    同じ「3秒加圧→3秒Idle」で繰り返す。

    戻り値:
        search_state, changed_start_id

    changed_start_idは、加圧・減圧セルの組み合わせが変化したときの
    新しい開始ID。それ以外はNone。
    """
    if search_state is None:
        return None, None

    # 7セルすべての比較試験が終わるまでは探索を完了させる。
    # 加圧・減圧セルの変化判定は、最良セルの繰り返し加圧中だけ行う。
    search_phase = search_state["phase"]
    if search_phase in ("repeat_pressurize", "repeat_idle"):
        latest_start_id = find_initial_start_id(markers)
    else:
        latest_start_id = None
    reference_start_id = search_state["reference_start_id"]

    if latest_start_id is not None and latest_start_id != reference_start_id:
        jump = cell_distance_circular(reference_start_id, latest_start_id)

        if jump < MAX_START_ID_JUMP:
            if latest_start_id == search_state["change_candidate_start_id"]:
                search_state["change_candidate_count"] += 1
            else:
                search_state["change_candidate_start_id"] = latest_start_id
                search_state["change_candidate_count"] = 1

            if (
                search_state["change_candidate_count"]
                >= RECOVERY_CONTROL_CHANGE_STABLE_FRAMES
            ):
                print(
                    f"加圧・減圧セルの変化を検出しました: "
                    f"開始セル{reference_start_id} → 開始セル{latest_start_id}"
                )
                return search_state, latest_start_id
        else:
            search_state["change_candidate_start_id"] = None
            search_state["change_candidate_count"] = 0
    else:
        search_state["change_candidate_start_id"] = None
        search_state["change_candidate_count"] = 0

    now = time.time()
    elapsed = now - search_state["phase_start_time"]
    phase = search_state["phase"]
    pressurize_cells = search_state["pressurize_cells"]
    depressurize_cells = search_state["depressurize_cells"]

    if phase == "wait_after_depressurize":
        if elapsed < DEPRESSURIZE_INTERVAL_SEC:
            return search_state, None

        cell_id = pressurize_cells[0]
        current_centroid = calculate_marker_centroid(markers)
        search_state["test_start_centroid_x"] = (
            current_centroid[0]
            if current_centroid is not None
            else None
        )

        print(f"探索試験: セル{cell_id}を加圧してください")
        send_cell_command(
            pressurize_cells=[cell_id],
            depressurize_cells=depressurize_cells,
        )
        search_state["phase"] = "test_pressurize"
        search_state["phase_start_time"] = now
        return search_state, None

    if phase == "test_pressurize":
        if elapsed < RECOVERY_SEARCH_PRESSURIZE_SEC:
            return search_state, None

        cell_id = pressurize_cells[search_state["test_index"]]
        print(f"探索試験: セル{cell_id}をアイドル状態にしてください")
        send_cell_command(depressurize_cells=depressurize_cells)
        search_state["phase"] = "test_idle"
        search_state["phase_start_time"] = now
        return search_state, None

    if phase == "test_idle":
        if elapsed < RECOVERY_SEARCH_IDLE_SEC:
            return search_state, None

        test_index = search_state["test_index"]
        cell_id = pressurize_cells[test_index]
        start_centroid_x = search_state["test_start_centroid_x"]
        current_centroid = calculate_marker_centroid(markers)
        current_centroid_x = (
            current_centroid[0]
            if current_centroid is not None
        ...
        if start_centroid_x is not None and current_centroid_x is not None:
            # 画像ではX座標が小さくなるほど左方向なので、
            # 「開始X－終了X」を左方向への重心移動量とする。
            moved_left_px = start_centroid_x - current_centroid_x
            search_state["scores"][cell_id] = moved_left_px
            print(
                f"探索結果: セル{cell_id}による重心の左方向移動量 "
                f"{moved_left_px:+.2f} px "
                f"（X: {start_centroid_x:.2f} → {current_centroid_x:.2f}）"
            )
        else:
            print(f"探索結果: セル{cell_id}の重心移動量を取得できませんでした")

        next_index = test_index + 1

        if next_index < len(pressurize_cells):
            next_cell = pressurize_cells[next_index]
            search_state["test_index"] = next_index
            search_state["test_start_centroid_x"] = current_centroid_x

            print(f"探索試験: セル{next_cell}を加圧してください")
            send_cell_command(
                pressurize_cells=[next_cell],
                depressurize_cells=depressurize_cells,
            )
            search_state["phase"] = "test_pressurize"
            search_state["phase_start_time"] = now
        ...
        if len(search_state["scores"]) == 0:
            print("移動量を取得できなかったため、7セルの探索をやり直します")
            search_state["test_index"] = 0
            search_state["phase"] = "wait_after_depressurize"
            search_state["phase_start_time"] = now
            return search_state, None

        best_cell = max(
            search_state["scores"],
            key=search_state["scores"].get,
        )
        best_moved_left_px = search_state["scores"][best_cell]
        search_state["best_cell"] = best_cell

        print(
            f"重心を最も左方向へ移動させたセルは{best_cell}です "
            f"（{best_moved_left_px:+.2f} px）"
        )
        print(f"セル{best_cell}の繰り返し加圧を開始します")
        send_cell_command(
            pressurize_cells=[best_cell],
            depressurize_cells=depressurize_cells,
        )
        search_state["phase"] = "repeat_pressurize"
        search_state["phase_start_time"] = now
        return search_state, None

    if phase == "repeat_pressurize":
        if elapsed < RECOVERY_SEARCH_PRESSURIZE_SEC:
            return search_state, None

        best_cell = search_state["best_cell"]
        print(f"セル{best_cell}をアイドル状態にしてください")
        send_cell_command(depressurize_cells=depressurize_cells)
        search_state["phase"] = "repeat_idle"
        search_state["phase_start_time"] = now
        return search_state, None

    if phase == "repeat_idle":
        if elapsed < RECOVERY_SEARCH_IDLE_SEC:
            return search_state, None

        best_cell = search_state["best_cell"]
        print(f"セル{best_cell}を再び加圧してください")
        send_cell_command(
            pressurize_cells=[best_cell],
            depressurize_cells=depressurize_cells,
        )
...
while True:
    try:
        target_distance_cm = float(input("目標移動距離を入力してください [cm]: "))
...
print(f"目標移動距離: {target_distance_cm:.2f} cm")
...
# 同じ加圧・減圧セルの組み合わせで終了した内部サイクル数
stalled_cycle_count = 0
stall_reference_control = None

# 加圧・減圧セル停滞時の復帰動作状態
recovery_state = None

# 復帰加圧後の最適セル探索状態
recovery_search_state = None
...
try:
    while True:
        ...
        if step1_cleared:
            ...
            # 毎フレーム、現在の累積移動距離を計算して目標距離と比較する。
            moved_intervals, current_distance_cm = calculate_boundary_distance(
                boundary_tracker
            )

            if current_distance_cm is not None:
                if abs(current_distance_cm) >= target_distance_cm:
                    target_reached = True

                    print(
                        f"目標距離に到達しました: "
                        f"目標={target_distance_cm:.2f} cm, "
                        f"現在={abs(current_distance_cm):.2f} cm"
                    )

                    # 目標到達フレームで直ちに全セルをIdleへ戻す。
        ...
        if recovery_state is not None:
            ...
            if recovery_finished:
                ...
                # 復帰加圧後の最新画像から、現在の加圧・減圧セルを決める。
                recovery_start_id = find_initial_start_id(markers)
                if recovery_start_id is None:
                    recovery_start_id = current_start_id

                current_start_id = recovery_start_id
                recovery_search_state = start_recovery_search(
                    current_start_id,
                    markers,
                )
                motion_reference = capture_motion_reference(markers)

        if recovery_search_state is not None:
            recovery_search_state, changed_start_id = run_recovery_search(
                recovery_search_state,
                markers,
            )

            if changed_start_id is not None:
                # 探索中に加圧・減圧セルが変化したら、
                # 新しい組み合わせで通常回転制御へ戻る。
                current_start_id = changed_start_id
                recovery_search_state = None
                stalled_cycle_count = 0
                stall_reference_control = None
                sequence_number += 1
        ...
        if (
            ...
            and recovery_search_state is None
        ...
        if one_cycle_finished:
            ...
            # 次に使用する開始セルを基準に、加圧・減圧セルの組み合わせを作る。
            # サイクル終了時に新しい開始セルが見つかった場合は、
            # その新しい組み合わせを使って停滞判定する。
            control_start_id = (
                pending_start_id
                if pending_start_id is not None
                else current_start_id
            )
            control_pressurize, control_depressurize = make_cells_from_start_id(
                control_start_id
            )
            current_control = (
                tuple(control_pressurize),
                tuple(control_depressurize),
            )

            if stall_reference_control is None:
                stall_reference_control = current_control
                stalled_cycle_count = 1
            elif current_control == stall_reference_control:
                stalled_cycle_count += 1
            else:
                print("加圧・減圧セルが更新されました")
                stall_reference_control = current_control
                stalled_cycle_count = 1

            print(
                f"同じ加圧・減圧セルでの経過サイクル数: "
    ...
    # 目標到達時は到達フレームですでにIdle送信済みなので二重送信しない。
    if hidas_sock is not None and not target_reached:
        try:
            print("全セルをアイドル状態にします")
            send_all_idle()
