...
def get_hidas_center_from_markers(markers):
    """検出マーカー中心の平均からHIDAS中心を求める。"""
    if not markers:
        return None

    cx = sum(m["center"][0] for m in markers) / len(markers)
    cy = sum(m["center"][1] for m in markers) / len(markers)
    return (cx, cy)


def get_largest_marker_scale(markers):
    """
    毎フレーム、外径が最大のマーカーを代表マーカーとして選び、
    実寸10cmから cm/px の縮尺を求める。

    戻り値:
        (cm_per_px, representative_marker)
    """
    valid_markers = [
        m for m in markers
        if m.get("outer_radius") is not None and m["outer_radius"] > 0
    ]

    if not valid_markers:
        return None, None

    representative_marker = max(
        valid_markers,
        key=lambda m: math.pi * (m["outer_radius"] ** 2)
    )

    diameter_px = representative_marker["outer_radius"] * 2.0

    if diameter_px <= 0:
        return None, None

    cm_per_px = MARKER_DIAMETER_CM / diameter_px
    return cm_per_px, representative_marker


def initialize_scale_distance_tracker(markers):
    """
    距離計測開始時のHIDAS中心を記録する。
    縮尺は毎フレーム最大マーカーから更新する。
    """
    hid_center = get_hidas_center_from_markers(markers)
    cm_per_px, representative_marker = get_largest_marker_scale(markers)

    if hid_center is None or cm_per_px is None:
        return None

    marker_id = representative_marker.get("assigned_id", -1)
    diameter_px = representative_marker["outer_radius"] * 2.0

    print(
        f"移動距離推定開始（マーカー縮尺法）: "
        f"代表マーカーID={marker_id}, "
        f"直径={diameter_px:.1f}px, "
        f"縮尺={cm_per_px:.4f}cm/px"
    )

    return {
        "start_center_x": hid_center[0],
        "start_center_y": hid_center[1],
        "current_center_x": hid_center[0],
        "current_center_y": hid_center[1],
        "cm_per_px": cm_per_px,
        "representative_marker_id": marker_id,
        "representative_marker_diameter_px": diameter_px,
        "distance_cm": 0.0,
    }


def update_scale_distance_tracker(markers, tracker):
    """
    毎フレーム、
    1. HIDAS中心を求める
    2. 最大マーカーを代表マーカーとして縮尺を更新する
    3. 開始時中心から現在中心までのX方向移動量をcmへ変換する

    画像左方向を正、右方向を負とする。
    """
    if tracker is None:
        return initialize_scale_distance_tracker(markers)
    ...
    tracker["current_center_x"] = hid_center[0]
    tracker["current_center_y"] = hid_center[1]
    tracker["cm_per_px"] = cm_per_px
    tracker["representative_marker_id"] = representative_marker.get("assigned_id", -1)
    tracker["representative_marker_diameter_px"] = (
        representative_marker["outer_radius"] * 2.0
    )

    # 左方向へ進むとX座標が小さくなるので、左方向を正にする。
    dx_px = tracker["start_center_x"] - tracker["current_center_x"]
    tracker["distance_cm"] = dx_px * cm_per_px

    return tracker


def calculate_scale_distance(tracker):
    """マーカー縮尺法で求めた現在の移動距離[cm]を返す。"""
    if tracker is None:
        return None
    return tracker.get("distance_cm")


def print_scale_distance(tracker, label="現在"):
    """マーカー縮尺法による移動距離を表示する。"""
    distance_cm = calculate_scale_distance(tracker)

    if distance_cm is None:
        print(f"{label}の推定移動距離: まだ計算できていません")
        return
...
try:
    while True:
        ...
        if step1_cleared:
            ...
            # 距離そのものは、最大マーカーの10cm縮尺と
            # HIDAS中心のX方向移動量から算出する。
            scale_distance_tracker = update_scale_distance_tracker(
                markers,
                scale_distance_tracker
            )

            current_distance_cm = calculate_scale_distance(scale_distance_tracker)
        ...
        if waiting_for_motion_result:
            ...
            if now - motion_result_wait_start >= COMMAND_INTERVAL_SEC:
                ...
                print_scale_distance(scale_distance_tracker, label="回転開始から現在まで")
    ...
    print("\nCtrl+Cによる中断を受け付けました")
    print_scale_distance(scale_distance_tracker, label="中断時点")

finally:
    # ESC・Ctrl+C・例外終了のいずれでも最終結果を表示する。
    print_scale_distance(scale_distance_tracker, label="終了時点")
