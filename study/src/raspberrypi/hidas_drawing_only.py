...
def get_bottom_continuous_position(markers):
    """
    HIDAS中心から見て、画像の真下（-90 deg）を挟む2つのマーカーを求める。

    戻り値:
        start_id, end_id, fraction などを格納した辞書。
        fraction=0.0 はstart側、1.0はend側。
    """
    marker_by_id = {
        m["assigned_id"]: m
        for m in markers
...
def draw_bottom_reference(image, markers, hid_center):
    ...
    # HIDAS中心から画像下端まで赤い真下基準線を描画
    cv2.line(image, (hx, hy), (hx, height - 1), (0, 0, 255), 3)
