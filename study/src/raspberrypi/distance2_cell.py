...
def get_bottom_interval(markers):
    """
    真下方向(BOTTOM_ANGLE_DEG)が、どの隣接セルID間にあるかを求める。
    ...
    for start_id in range(1, 17):
        end_id = (start_id % 16) + 1

        if start_id not in angle_by_id or end_id not in angle_by_id:
            continue

        start_angle = angle_by_id[start_id]
        end_angle = angle_by_id[end_id]

        # IDの並びに合わせ、start_idからend_idへ時計回りに見た区間を使う。
        interval_angle = (start_angle - end_angle) % 360.0
        bottom_offset = (start_angle - BOTTOM_ANGLE_DEG) % 360.0

        if bottom_offset <= interval_angle:
            return start_id

    return None


def initialize_boundary_tracker():
    """真下セル区間の通過回数を数える状態を初期化する。"""
    return {
        "initialized": False,
        ...
        "candidate_frames": 0,
        "crossed_intervals": 0,
        "last_detected_interval": None
    }


def update_boundary_tracker(markers, tracker):
    """
    真下区間が安定して次の区間へ移ったときだけ、通過区間数を更新する。
    一時的な誤検出や2区間以上の急変は移動量に加算しない。
    """
    detected_interval = get_bottom_interval(markers)
    tracker["last_detected_interval"] = detected_interval

    if detected_interval is None:
        tracker["candidate_interval"] = None
        tracker["candidate_frames"] = 0
        return tracker

    if not tracker["initialized"]:
        if tracker["candidate_interval"] == detected_interval:
            tracker["candidate_frames"] += 1
        else:
            tracker["candidate_interval"] = detected_interval
            tracker["candidate_frames"] = 1

        if tracker["candidate_frames"] >= BOTTOM_INTERVAL_STABLE_FRAMES:
            tracker["confirmed_interval"] = detected_interval
            tracker["initialized"] = True
            tracker["candidate_interval"] = None
            tracker["candidate_frames"] = 0
            print(
                f"移動距離推定開始: 真下区間は"
                f"セル{detected_interval}-セル{(detected_interval % 16) + 1}間"
            )
        return tracker

    if detected_interval == tracker["confirmed_interval"]:
        tracker["candidate_interval"] = None
        tracker["candidate_frames"] = 0
        return tracker
    ...
    previous_interval = tracker["confirmed_interval"]

    # 1→2を+1、1→16を-1として循環差を計算する。
    forward_diff = (detected_interval - previous_interval) % 16
    if forward_diff > 8:
        signed_diff = forward_diff - 16
    else:
        signed_diff = forward_diff

    if abs(signed_diff) <= MAX_BOTTOM_INTERVAL_JUMP:
        tracker["crossed_intervals"] += signed_diff
        tracker["confirmed_interval"] = detected_interval
        print(
            f"真下区間通過: セル{previous_interval}-セル{(previous_interval % 16) + 1}間"
            f" → セル{detected_interval}-セル{(detected_interval % 16) + 1}間"
        )
    else:
        print(
            f"真下区間の急変を無視しました: "
            f"{previous_interval} → {detected_interval}"
        )
...
def calculate_boundary_distance_cm(tracker):
    """通過したセル区間数を距離[cm]へ変換する。"""
    return tracker["crossed_intervals"] * CELL_LENGTH_CM


def print_boundary_distance(tracker, label="現在"):
    """真下セル通過法による移動距離を表示する。"""
    if tracker is None or not tracker["initialized"]:
        print(f"{label}の推定移動距離: まだ真下区間を確定できていません")
        return

    intervals = tracker["crossed_intervals"]
    distance_cm = calculate_boundary_distance_cm(tracker)

    if intervals > 0:
        direction = "区間番号が増える方向"
    elif intervals < 0:
        direction = "区間番号が減る方向"
    else:
        direction = "移動なし"
    ...
    print(
        f"{label}の推定移動距離（真下セル通過法）: "
        f"{abs(distance_cm):.1f} cm "
        f"（{abs(intervals)}セル区間、{direction}）"
    )
...
# 真下セル通過法の移動距離推定状態
boundary_tracker = initialize_boundary_tracker()
latest_markers = []
