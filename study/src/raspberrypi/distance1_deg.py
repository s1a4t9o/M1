...
def update_travel_tracker(markers, previous_angle_by_id, accumulated_intervals):
    """
    前フレームと現在フレームで共通するマーカーの角度変化量の中央値から、
    移動したセル区間数を積算する。
    ...
    common_ids = sorted(
        set(previous_angle_by_id.keys()) & set(current_angle_by_id.keys())
    )
    ...
    for cell_id in common_ids:
        delta_deg = normalize_angle_diff(
            current_angle_by_id[cell_id] - previous_angle_by_id[cell_id]
        )

        # ID付け替えなどによる大きな飛びは外れ値として除外する。
    ...
    # 1フレームで大きく移動した値は検出・ID対応異常として採用しない。
...
def format_travel_distance(accumulated_intervals):
    """積算区間数を方向付きの表示文字列にする。"""
    distance_cm = accumulated_intervals * CELL_INTERVAL_LENGTH_CM
    ...
    return direction, abs(accumulated_intervals), abs(distance_cm)


def print_estimated_travel_distance(accumulated_intervals, prefix="推定移動距離"):
    """現在までの推定移動距離を出力する。"""
    direction, intervals, distance_cm = format_travel_distance(accumulated_intervals)

    if direction == "移動なし":
        print(f"{prefix}: ほぼ0.0 cm（0.000セル区間）")
    else:
        print(
            f"{prefix}: {direction}に{distance_cm:.1f} cm "
            f"（{intervals:.3f}セル区間）"
        )
...
try:
    while True:
        ...
        # 回転開始後は、各フレームのマーカー角度変化から総移動距離を積算する。
        if step1_cleared:
            if not travel_tracking_started:
                travel_previous_angle_by_id = get_marker_angle_map(markers)
                travel_tracking_started = True
            else:
        ...
        if waiting_for_motion_result:
            ...
            if now - motion_result_wait_start >= COMMAND_INTERVAL_SEC:
                ...
                print_estimated_travel_distance(
                    accumulated_travel_intervals,
                    prefix="回転開始からの推定移動距離"
                )
    ...
    if travel_tracking_started:
        ...
        print("終了時点の推定移動距離: 回転開始前のため未計測")

    # 安全のため、接続が残っていれば全セルをIdleに戻す。
    if hidas_sock is not None:
        try:
            send_all_idle()
        except (OSError, RuntimeError) as exc:
            print(f"終了時のIdle指示に失敗しました: {exc}")
        finally:
            try:
                hidas_sock.close()
            except OSError:
